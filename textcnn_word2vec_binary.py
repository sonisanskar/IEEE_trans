...
data_train=pd.read_csv('../input/disaster-tweets/tweets.csv')

data_test=pd.read_csv('../input/disaster-tweets/tweets.csv')
...
data_train.rename(columns={'text':'title','target':'tag'},inplace=True)
data_test.rename(columns={'text':'title','target':'tag'},inplace=True)
#print(data_train['Review Text'])
#data_train['title']=data_train['Review Text'][:]
#data_train['title']=data_test['Review Text'][:]
#print(type(data_train['title'][0]))
...
# In[8]:
#data_train.rename(columns={'text':'title'},inplace=True)
#data_test.rename(columns={'text':'title'},inplace=True)
...
#data_train.rename(columns={'sentiment':'tag'},inplace=True)
#data_test.rename(columns={'sentiment':'tag'},inplace=True)
...
filters=[32,48,64,128]neurons=[64,128]
rate=[0.4,0.5]
rates=[0.4,0.5]
...
param_grid = dict( neurons = neurons,rates=rates,rate=rate,filters=filters,learning_rate=learning_rate,epochs=epochs,batch_size=batch_size)

#clf = KerasClassifier(build_fn= get_model,verbose= 0)
#print('started tuning')
#from sklearn.model_selection import GridSearchCV
#model09 = GridSearchCV(estimator= clf, param_grid=param_grid, n_jobs=-1)
#model09.fit(train_cnn_data,y_train)
#print(model09.best_params_)'''
