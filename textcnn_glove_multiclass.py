...
data_train=pd.read_csv('../input/dbpedia-classes/DBPEDIA_train.csv')

data_test=pd.read_csv('../input/dbpedia-classes/DBPEDIA_test.csv')
...
data_train.rename(columns={'text':'title','l1':'tag'},inplace=True)
data_test.rename(columns={'text':'title','l1':'tag'},inplace=True)
...
history = model.fit(train_cnn_data, y_train,  epochs=10,callbacks=callbacks_list,validation_split=0.1 )

print(type(train_cnn_data[0]))
...
print('the accuracy')
print(accuracy_score(original_ans,output_class_pred))
...
early_stopping = EarlyStopping(monitor='val_loss', min_delta=0.01, patience=4, verbose=1)
callbacks_list = [early_stopping]
history1 = model.fit(train_cnn_data, y_train,  epochs=10,callbacks=callbacks_list,validation_split=0.1 )
...
if(m<5):
    ...
    m+=2
print('x1,x2,y1,y2',x1,x2,y1,y2)# In[162]:
...
early_stopping = EarlyStopping(monitor='val_loss', min_delta=0.01, patience=4, verbose=1)
callbacks_list = [early_stopping]
history2 = model1.fit(training_data, y_train,  epochs=10,callbacks=callbacks_list,batch_size=32,validation_split=0.1 )
...
plt.ylabel('loss')
plt.xlabel('epoch')
plt.legend(['TextConvoNet_6', 'TextConvoNet_4',"YoonKim"], loc='upper right')
plt.show(block=True)
...
plt.ylabel('loss')
plt.xlabel('epoch')
plt.legend(['TextConvoNet_6', 'TextConvoNet_4',"YoonKim"], loc='upper left')
plt.show(block=True)
...
name="text_6"
print('TEXTCONVONET_6')
print(history.history['loss'])
print(history.history['val_loss'])
print(history.history['accuracy'])
print(history.history['val_accuracy'])

data={"name":name,'loss':history.history['loss'],"val_loss":history.history['val_loss'],"training_accuracy":history.history['accuracy'],"val_acc":history.history['val_accuracy']}
metric=pd.DataFrame(data)
metric.to_csv('experiment.csv', mode='a', index = False, header=metric.columns,columns=metric.columns)

name="text_4"
print('TEXTCONVONET_4')
...
data={"name":name,'loss':history1.history['loss'],"val_loss":history1.history['val_loss'],"training_accuracy":history1.history['accuracy'],"val_acc":history1.history['val_accuracy']}
metric=pd.DataFrame(data)
metric.to_csv('experiment.csv', mode='a', index = False, header=metric.columns,columns=metric.columns)
...
data={"name":name,'loss':history2.history['loss'],"val_loss":history2.history['val_loss'],"training_accuracy":history2.history['acc'],"val_acc":history2.history['val_acc']}
metric=pd.DataFrame(data)
metric.to_csv('experiment.csv', mode='a', index = False, header=metric.columns,columns=metric.columns)
